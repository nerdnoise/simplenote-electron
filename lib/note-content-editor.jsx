...
const indentSelection = ( content, { selectionStart, selectionEnd } ) => {
	...
	const startOffset = selectionStart !== selectionEnd && content[ selectionStart ] === '\n'
		? selectionStart - 1
		: selectionStart;
...
const outdentSelection = ( content, { selectionStart, selectionEnd } ) => {
	const startOffset = content[ selectionStart ] === '\n'
		? selectionStart - 1
		: selectionStart;

	const prevNewline = content.lastIndexOf( '\n', startOffset );
	const lineStart = Math.max( 0, prevNewline );
	const firstChar = lineStart === 0
		? content[ 0 ]
		: content[ lineStart + 1 ];
	...
	const transformed = before + selection.replace( /^\t/, '' ).replace( /\n\t/g, '\n' ) + after;
	const delta = transformed.length - content.length;
...
export default class NoteContentEditor extends React.Component {
	...
	handleEnter = event => {
		...
		const transformed = addListBullet( content, { selectionStart, selectionEnd } );
	...
	handleTab = event => {
		...
		this.setState( { content: transformed }, () => this.input.setSelectionRange(
	...
	changeContent = ( { target: { value } } ) => setTimeout( () => this.setState( {
		content: value
	}, () => this.props.onChangeContent( value ) ), 3000 );
